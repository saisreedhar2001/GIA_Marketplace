    ...
    const fetchUserData = async () => {
        try {
            ...
            const [ordersRes, portfolioRes] = await Promise.all([
                api.get('/orders'),
                user?.role === 'admin' ? api.get(`/products?artistId=${user.id}`) : Promise.resolve({ data: { items: [] } }),
            ])
    ...
    return (
        <div className="bg-off-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                {/* Tabs */}
                <div className="flex gap-4 mb-8 border-b border-sand-beige overflow-x-auto">
                    {(user?.role === 'admin'
                        ? ['profile', 'portfolio', 'orders', 'subscription']
                        : ['profile', 'orders', 'subscription']
                    ).map((tab) => (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab as any)}
                            className={`px-4 py-3 font-medium transition whitespace-nowrap ${activeTab === tab
                ...
                {activeTab === 'profile' && (
                    <div className="card max-w-2xl">
                        ...
                        <div className="space-y-4">
                            ...
                            {user.role === 'admin' && (
                                <div className="pt-4 border-t border-sand-beige">
                                    <p className="text-sm text-warm-gray mb-2">Portfolio</p>
                                    <a
                                        href="/my-portfolio"
                ...
                {/* Portfolio Tab */}
                {activeTab === 'portfolio' && user?.role === 'admin' && (
                    <div className="text-center py-12">
                        <div className="mb-6">
                            ...
                            <p className="text-warm-gray mb-8">
                                Showcase {portfolio.length} work{portfolio.length !== 1 ? 's' : ''}
                            </p>
                        </div>
                        <a
                            href="/my-portfolio"
                            className="inline-block px-8 py-4 bg-terracotta text-off-white rounded-lg font-semibold hover:bg-opacity-90 transition text-lg"
                        >
                            View Full Portfolio →
                        </a>
                    </div>
